          ...
          <div className="flex justify-between items-center py-4">
            ...
            <div className="flex items-center space-x-1 sm:space-x-2">
              <nav className="hidden sm:flex items-center space-x-1">
                <Link to="/explore" className="flex items-center text-sm sm:text-base text-text-main-light dark:text-text-main-dark hover:text-primary-light dark:hover:text-primary-dark transition-colors px-3 py-2 rounded-md">
                  <Compass size={18} className="mr-1 sm:mr-2" /> {translate('Explorar')}
                </Link>
                <Link to="/settings" className="flex items-center text-sm sm:text-base text-text-main-light dark:text-text-main-dark hover:text-primary-light dark:hover:text-primary-dark transition-colors px-3 py-2 rounded-md">
              ...
              {/* Ícone de Restrição de Idade - Posicionado discretamente */}
              ...
              {!loading && (
                user ? (
                  <div className="flex items-center space-x-2">
                    ...
                    <button onClick={logout} className="px-3 py-2 text-sm bg-red-600 text-white rounded-md hover:bg-red-700">
                      Sair
                    </button>
                  </div>
                ) : (
                  <button onClick={signInWithGoogle} className="px-3 py-2 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700">
                    Login Google
                  </button>
                )
              )}
              ...
              <div className="flex space-x-1">
                {Object.keys(languages).map((langKey) => (
                  <button key={langKey} onClick={() => setLanguage(langKey)} className={`px-2 py-1 rounded-md text-xs font-semibold ${language === langKey ? 'bg-primary-light text-white' : 'bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-200'}`}>
                    {langKey.toUpperCase()}
                  </button>
                ))}
              </div>
